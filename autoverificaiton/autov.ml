...
let check vc = _check (Prop.negate vc)
let check_implies a b = _check Prop.(negate (Implies (a, b)))
let check_implies_multi_pre a_s b = _check Prop.(negate (Implies (And a_s, b)))
