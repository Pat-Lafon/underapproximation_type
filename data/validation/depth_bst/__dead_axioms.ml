...
let tree_leaf_or_not (l : int tree) = leaf l || not (leaf l)
...
(* let[@axiom] tree_non_leaf_has_lower (l : int tree) ((x [@exists]) : int) =
  (not (leaf l))#==>(lower_bound l x)

let[@axiom] tree_non_leaf_has_upper (l : int tree) ((x [@exists]) : int) =
